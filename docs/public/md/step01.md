...
コンテナは 2 つだけです。どちらも nginx が静的ファイルを返すだけで、アプリケーションサーバも DB もまだありません。

```
                 ┌──────────────────────────┐
ブラウザ ────────▶│ frontend  :8080  (nginx) │  ビューア本体 + ページ画像
                 └──────────────────────────┘
                 ┌──────────────────────────┐
                 │ docs      :8081  (nginx) │  このドキュメント
                 └──────────────────────────┘
                 ┌ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┐
                 │  backend  :8000          │  compose.yaml にコメントで確保
                 └ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┘
```

| サービス | ポート | 中身 | 役割 |
|---|---|---|---|
| `frontend` | 8080 | nginx（`./frontend` を build） | ビューア本体。ページ画像も同じコンテナから配る |
| `docs` | 8081 | nginx（公式イメージのまま） | このドキュメントの閲覧 |
| `backend` | — | — | `compose.yaml` にコメントで追加箇所だけ確保した状態 |

**画像もフロントエンドと同じコンテナから配っています。** ページ画像は `frontend/public/assets/pages/page-NN.svg` に置き、`reader.js` の `CONFIG.pageSrc(n)` が直接参照します。API も CDN も無いため、この段階はフロントエンド単体で完結しています。

`docs` だけ build していないのは、**イメージに焼き込むものが無い**からです。静的ファイルと `nginx.conf` をマウントするだけで足りるので、`nginx:1.27-alpine` をそのまま使っています。

ページ画像の生成に使う `tools/generate_dummy_pages.py` は、コンテナではなく**ホストで `python3` を実行**するスクリプトです。
