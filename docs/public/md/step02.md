...
| サービス | ポート | 中身 | 役割 |
|---|---|---|---|
| `frontend` | 8080 | nginx（`./frontend` を build） | ビューア本体（HTML/CSS/JS） |
| `backend` | 8000 | php:8.4-cli-alpine + `artisan serve` | 書誌情報・ページ一覧を JSON で返す |
| `cdn` | 8082 | nginx（公式イメージのまま） | ページ画像。圧縮・長期キャッシュの設定付き |
| `docs` | 8081 | nginx（公式イメージのまま） | このドキュメントの閲覧 |

Step.01 から増えたのは `backend` と `cdn` の 2 つです。`backend` だけ `Dockerfile` を持たせており、`pdo_mysql` を先に入れてあります（DB を入れる段階で使います）。`cdn` と `docs` は静的ファイルと `nginx.conf` をマウントするだけなので、build せず公式イメージをそのまま使っています。
